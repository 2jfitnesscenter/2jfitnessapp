...
2J Fitness Center ships in two flavors from the same codebase:
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which 2J Fitness Center doesn't do.
...
2J Fitness Center's mobile app is not on the Play Store or App Store, and that's a choice: no store
accounts, no store rules, no yearly fees between you and an open-source app.

### Android — sideload the APK

Distribute your signed APK from wherever you host your own downloads.
...
keytool -genkeypair -keystore my.keystore -alias 2jfitness -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias 2jfitness --out 2JFitnessCenter.apk aligned.apk
```
