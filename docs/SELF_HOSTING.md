# Self-hosting 2J Fitness Center

2J Fitness Center is two small containers (a web server and an API) plus a folder of your data.
...
```bash
git clone <your-repository-url> 2jfitness
cd 2jfitness
cp .env.example .env
...
2J Fitness Center signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use 2J Fitness Center from your phone you need a real HTTPS hostname.
...
Put 2J Fitness Center behind something that terminates TLS for a hostname you control, then point it at
...
2J Fitness Center only needs the browser to reach it over `https://gym.example.com`.
...
```bash
tar czf 2jfitness-backup-$(date +%F).tar.gz data/
```
...
2J Fitness Center can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
2. Pick one built-in provider:
   - **Claude Code**: on a trusted computer where you use Claude Code, run `claude setup-token`.
     ...
     in 2J Fitness Center. The token is encrypted at rest and passed only to the isolated Agent SDK job.
   - **OpenAI Codex CLI**: choose **Sign in with ChatGPT**, then use the link and one-time code
     on a trusted browser or iPad. This is Codex's device-code login; 2J Fitness Center never receives a
