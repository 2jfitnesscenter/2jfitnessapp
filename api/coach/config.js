/* Coach instance configuration — the one place that knows whether this instance offers the
   ...
   cache, rather than 2J Fitness Center importing or duplicating an OAuth token. */
...
// This file is deliberately owned by 2J Fitness Center, not a person's normal Codex configuration.
// It gives spawned agent commands only their temporary workspace and the minimal runtime paths;
// specifically, `:root = "deny"` prevents a Coach prompt from reading /codex/auth.json.
const COACH_CODEX_CONFIG = `# Managed by 2J Fitness Center; do not add plugins, MCP servers, or broad permissions here.\ncli_auth_credentials_store = "file"\nallow_login_shell = false\napproval_policy = "never"\nweb_search = "disabled"\ndefault_permissions = "coach"\n\n[features]\nauth_elicitation = false\nshell_tool = false\nunified_exec = false\nshell_snapshot = false\nbrowser_use = false\nbrowser_use_external = false\nbrowser_use_full_cdp_access = false\nin_app_browser = false\ncomputer_use = false\napps = false\nplugins = false\nplugin_sharing = false\nremote_plugin = false\nmulti_agent = false\nskill_search = false\nskill_mcp_dependency_install = false\nworkspace_dependencies = false\nimage_generation = false\nhooks = false\ncode_mode_host = false\ntool_suggest = false\n\n[agents]\nenabled = false\n\n[permissions.coach.filesystem]\n":root" = "deny"\n":minimal" = "read"\n\n[permissions.coach.filesystem.":workspace_roots"]\n"." = "read"\n\n[permissions.coach.network]\nenabled = false\n`;
...
export function jobEnv(jobDir) {
  ...
  if (cfg.provider === 'codex') {
    ...
    // bind mount owned by `coach`; /data and every 2J Fitness Center secret remain inaccessible.
