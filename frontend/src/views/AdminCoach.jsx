...
/* The operator's side of the Coach: is it on, can it reach a model, and what has it been
   doing. Uses the same t()/locale packs as the rest of the admin dashboard.
 *
...
export default function AdminCoach() {
  ...
  if (d.disabledByEnv) return <div className="card">
    ...
    <div className="muted small">{t('Force-disabled by')} <code>COACH_DISABLED</code> {t('in the environment. Remove it to configure the Coach here.')}</div>
  </div>
      ...
      {(meta.setupToken || meta.deviceLogin || meta.apiKey) && <>
        ...
        {d.auth?.state === 'connected' ? <>
          <div className="small muted" style={{ marginBottom: 8 }}>
            {d.auth.account ? t('Connected as {0} via {1} · {2}', d.auth.account, credentialLabel(d.auth.type), rel(d.auth.connectedAt)) : t('Connected via {0} · {1}', credentialLabel(d.auth.type), rel(d.auth.connectedAt))}
          </div>
